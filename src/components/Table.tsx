...
function Table<T extends DefaultRecordType>(props: Props<T>) {
  const { rows, columns, dataSource, placeholder = undefined } = props
  ...
  return <div className={styles.wrapper}>
      <table className={cx({ margin: colspan })}>
        ...
        <tbody>
          { bound(dataSource.length ? dataSource.map((record, index) => {
            ...
            return (
              <tr
                className={classNames(cx(rows?.(record).background), styles.tr)}
                key={index}
              >
