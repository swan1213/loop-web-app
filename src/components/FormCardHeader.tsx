...
const FormCardHeader: FC<Props> = ({ header, title, ...props }) => {
  const { icon, description, goBack, action, loading, slippage, headerClass } = props
  ...
  return !(header || title) ? null : (
    <header className={classNames(headerClass ? headerClass : styles.header, className)}>
      <span className={styles.title}>{header ?? render}</span> {slippage}
    </header>
  )
}
